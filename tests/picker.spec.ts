...
const makePicker = (plugin: PeriodicTypesPlugin): NotePickerModal =>
    new NotePickerModal({} as App, plugin);
...
const openNoteSpy = (plugin: PeriodicTypesPlugin): Mock =>
    (plugin as unknown as { openNote: Mock }).openNote;
...
describe('NotePickerModal — period step', () => {
    ...
    it('should open the next periodic note when a period is chosen', () => {
        ...
        expect(openNoteSpy(plugin)).toHaveBeenCalledWith(
            type,
            expect.objectContaining({ delta: 1, granularity: 'day' }),
        );

...
vi.mock('obsidian', () => {
    ...
    class Setting {
        ...
        addButton(cb: (c: unknown) => void) {
            cb(
                Object.assign(chain<() => Promise<void>>(fn => captured.buttonOnClick.push(fn)), {
...
const containerDouble = (tab: NoteTypeSettingTab): ContainerDouble =>
    tab.containerEl as unknown as ContainerDouble;
...
const pluginSpies = (plugin: PeriodicTypesPlugin): { saveSettings: Mock; loadSettings: Mock; configureCommands: Mock } =>
...
describe('NoteTypeSettingTab — registry mutations', () => {
    it('should reset to the work seed and show a Notice when the last type is deleted', async () => {
        ...
        expect(noticeMessages).toEqual(
            expect.arrayContaining([expect.stringContaining('reset to the default')]),
        );
    });

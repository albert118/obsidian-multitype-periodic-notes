...
const commandSpies = (plugin: PeriodicTypesPlugin): CommandSpies =>
	plugin as unknown as CommandSpies;

/** The command ids passed to addCommand across the lifecycle. */
const addedIds = (plugin: PeriodicTypesPlugin): string[] =>
	commandSpies(plugin).addCommand.mock.calls.map(
...
describe("configureCommands", () => {
	...
	it("should keep the picker command stable across calls and never remove it", () => {
		...
		expect(commandSpies(plugin).removeCommand).not.toHaveBeenCalledWith(
			"periodic-types:open-periodic-picker"
		);
	});
...
describe("onload seeding", () => {
	...
	const saveDataSpy = (plugin: PeriodicTypesPlugin): Mock =>
		(plugin as unknown as { saveData: Mock }).saveData;
	...
	it("should seed the default work type and persist it when loadData resolves undefined", async () => {
		...
		expect(plugin.settings.types).toEqual([
			expect.objectContaining({ id: "work", name: "Work", folder: "Work" }),
		]);
	...
	it("should not seed or persist when loadData returns a non-empty types list", async () => {
		...
		plugin.loadData = vi
			.fn()
			.mockResolvedValue({ types: [makeNoteType({ id: "journal" })] });
		plugin.saveData = vi.fn().mockResolvedValue(undefined);

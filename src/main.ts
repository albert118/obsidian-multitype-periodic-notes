...
export function handleNoteError(
	kind: NoteErrorKind,
	ctx: NoteErrorContext,
	error: unknown
): void {
	console.error(
		`[periodic-types] ${kind} — type: ${ctx.typeId}, date: ${ctx.date}, path: ${ctx.path}`,
		error
	);
 ...
 */
function stampFrontmatterIfMissing(
...
export default class PeriodicTypesPlugin extends Plugin {
	...
	async openNote(type: NoteTypeConfig, date: MomentLike): Promise<void> {
		...
		if (!(file instanceof TFile)) {
			new Notice(
				`Can't open the '${type.id}' note — a folder occupies the path '${file?.path ?? path}'.`
			);
			return;
		}

...
export class NoteTypeSettingTab extends PluginSettingTab {
	...
	private renderTab(): void {
		...
		new Setting(containerEl)
			...
			.addText((text) =>
				text
					.setPlaceholder("E.g. Journal, meetings…")
					.onChange((value) => {
						newName.value = value;
					})
			)
			.addButton((btn) =>
				btn.setButtonText("Add").setCta().onClick(() => this.addType(newName.value))
			);
	}
	...
	private renderTypeSettings(parent: HTMLElement, type: NoteTypeConfig): void {
		...
		new Setting(parent)
			.setName("Name")
			.setDesc("Display name used in the settings tab and picker. Commands are labeled from the immutable ID — renaming never changes a command.")
			.addText((text) =>
		...
		new Setting(parent)
			...
			.setDesc(
				"Open today's note for this type when Obsidian launches (but not on a plugin reload)."
			)
			.addToggle((toggle) =>

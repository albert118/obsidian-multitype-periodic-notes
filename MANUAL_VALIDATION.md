...
- [x] Configure two types: `Work` → `Work/`, `Journal` → `Journal/`, each with a
      throwaway template using `{{date}}` / `{{time}}` / `{{title}}`.
- [x] "Open today's Work note" creates `Work/<today>.md`; tokens substituted;
      folder auto-created; frontmatter has `type: work` (+ `date:`).
- [x] Re-run "Open today's Work note" → opens the existing note in the current
      tab, does NOT duplicate.

### A2. Picker

- [x] "Open periodic note…" → type list appears → selecting a type advances to a
      period list (Today / Next / Previous) → selection opens the correct note.

### A3. Settings registry

- [x] Add a third type via settings → its "Open today's…" command appears without
...
- [x] Delete a type → its command disappears from the palette, no orphan.
- [x] Disable a type → its command disappears.
- [x] Add a type with an empty name → blocked with a Notice.
- [x] Add types with colliding slugs (e.g. "Work" then "Work!") → duplicate id
      gets a counter suffix (`work`, `work-2`).

### A4. Collision guard

- [x] Point two enabled types at the same rendered path (e.g. same folder + same
      format) → save blocked with a Notice naming both.
- [x] Distinct names in one folder (daily `YYYY-MM-DD` vs monthly `YYYY-MM`)
      allowed.

### A5. openAtStartup

- [x] Enable `openAtStartup` on one type, full Obsidian restart → today's note
      opens.
- [x] Plugin reload / disable-toggle (NOT a cold start) → does NOT pop notes.

### A6. Robustness

- [x] Fire two "open today" for the same type rapidly → no duplicate-note crash.
- [x] Create a folder named exactly like a target note → no-op, no throw (a
      short Notice explains the folder at that path).

### A7. Registry edge

- [x] Delete ALL types → "Reset to default" Notice and the Work block
      immediately reappears; reload → Work persists.
- [x] (Optional) Edit `data.json` to `{"types":[]}` and reload → `work` is
      re-seeded.
...
- [x] **B1. Command removal works live.** Delete a type and disable a type →
...
- [x] **B2. Rename-label behavior — RESOLVED (by design).** Command id AND label
...
- [x] **B4. Delete-all + reload** — see A7 (work re-seeded = correct after fix).
- [x] **B5. Templater interaction** (if Templater installed): a template with
      `<% %>` produces expected body; `{{date}}/{{time}}/{{title}}` still
      substitute; no double-render or mangled tokens.
- [x] **B6. Corrupt-YAML existing note.** Open a hand-made note at a type's path
...
- [x] **B7. Settings tab renders (latest Obsidian).** ⚠️ **Assume latest Obsidian
...
- [x] **B8. openAtStartup cold-start only (ACCEPTED).** ⚠️ **Confirmed by user** —
